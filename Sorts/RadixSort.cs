...
namespace ManySorts.Sorts
{
    internal class RadixSort
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("基数排序（Radix Sort）是一种非比较型整数排序算法，按位分组排序。\n时间复杂度O(d·(n+k))，空间复杂度O(n+k)。仅适合整数排序。\n");
            ...
            Console.WriteLine("开始 Radix Sort 排序...");
            ...
            string outputPath = "radixsort_sorted.txt";
            try
            {
        ...
        // Radix Sort 实现
        private static void RadixSortArray(int[] array)
        {
            int n = array.Length;
            int max = 0;
